...
def generate_control_file(version):
    ...
    content = """Package: ez2desk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: ez2desk <info@ez2desk.com>
Homepage: https://ez2desk.com
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/etc/ez2desk/')
    ...
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/ez2desk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/ez2desk/')
    system2(
        'cp ../res/pam.d/rustdesk.debian tmpdeb/etc/pam.d/rustdesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/ez2desk/files/polkit && chmod a+x tmpdeb/usr/share/ez2desk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb ez2desk.deb;')
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/ez2Desk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"RustDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon ez2Desk.app 200 190 --hide-extension ez2Desk.app rustdesk.dmg ./build/macos/Build/Products/Release/ez2Desk.app")
    os.rename("rustdesk.dmg", f"../ez2desk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    os.rename('./rustdesk_portable.exe', f'./ez2desk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/ez2desk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv ../../{res_dir}/rustdesk-{version}-win7-install.exe ../../ez2desk-{version}-win7-install.exe')
        ...
        system2('mv rustdesk-%s-0-x86_64.pkg.tar.zst ez2desk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        ...
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/ez2Desk.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/ez2Desk.app/Contents/MacOS/')
    ...
    '''.format(pa))
                system2(
                    'create-dmg "ez2Desk %s.dmg" "target/release/bundle/osx/ez2Desk.app"' % version)
                os.rename('ez2Desk %s.dmg' %
              version, 'ez2desk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    codesign -s "Developer ID Application: {0}" --force --options runtime ./ez2desk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple ez2desk-{1}.dmg
